...
class ViewController: UIViewController,UITableViewDelegate, UITableViewDataSource {
        ...
        do {
            ...
            for result in results as! [NSManagedObject]{
                if let landmarkName = result.value(forKey: "landmarkName") as? String {
                    self.nameArray.append(landmarkName)
                }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toDetailsVC" {
            ...
            destinationVC.chosenLandmark = selectedLandmark
            destinationVC.chosenLandmarkId = selectedLandmarkId
        }
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        selectedLandmark = nameArray[indexPath.row]
        selectedLandmarkId = idArray[indexPath.row]
        performSegue(withIdentifier: "toDetailsVC", sender: nil)
    }
